...
namespace Web {
    ...
    typedef std::unordered_map<std::string, std::map<std::string,
    ...
    class ServerBase {
    public:
        void addHandlers(std::string path, std::vector<std::string> methods, std::function<void(std::ostream&, Request&)> func) {
            for (auto method : methods) {
                resource[path][method] = func;
            }
        }
        void addDefaultHandlers(std::string path, std::vector<std::string> methods, std::function<void(std::ostream&, Request&)> func) {
            for (auto method : methods) {
        ...
        void start() {
            ...
            // if num_threads>1, then m_io_service.run() will start (num_threads-1) threads as thread pool
