...
void start_server(SERVER_TYPE& server) {
    ...
    server.addHandlers("^/string/?$", {"GET","POST"}, [](ostream& response, Request& request) {
    ...
    server.addHandlers("^/info/?$", { "GET","POST" }, [](ostream& response, Request& request) {
    ...
    server.addHandlers("^/match/([0-9a-zA-Z]+)/?$", { "GET","POST" }, [](ostream& response, Request& request) {
        string number = request.path_match[1];
        response << "HTTP/1.1 200 OK\r\nContent-Length: " << number.length() << "\r\n\r\n" << number;
    });

    // resources request
    server.addDefaultHandlers("^/?(.*)$", {"POST", "PUT" }, [](ostream& response, Request& request) {
        string number = request.path_match[1];
        number = "random path here!:" + number;
        response << "HTTP/1.1 200 OK\r\nContent-Length: " << number.length() << "\r\n\r\n" << number;
    });
    // peocess default GET request; anonymous function will be called if no other matches
    // response files in folder web/
    // default: index.html
    server.addDefaultHandlers("^/?(.*)$", { "GET" }, [](ostream& response, Request& request) {
        string filename = "www/";
